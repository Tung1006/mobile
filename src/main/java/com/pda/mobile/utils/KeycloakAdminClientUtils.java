...
import com.pda.mobile.configs.KeycloakAdminClientConfig;
import com.pda.mobile.configs.KeycloakPropertyReader;
...
//   
		return Keycloak.getInstance(config.getServerUrl(), config.getRealm(), config.getClientId(),
				session.getTokenString());

	}

	public static Keycloak getKeycloakInstanceUserPass(KeycloakAdminClientConfig config, String username,
			String password) {

		return Keycloak.getInstance(config.getServerUrl(), config.getRealm(), username, password, config.getClientId());
	}
